...
import com.ravinada.cryptocompare.modelclasses.HourlyHistoricalData;
import com.ravinada.cryptocompare.modelclasses.MinuteHistoricalData;
...
public class CoinDetailRepository {
    ...
    public List<HourlyHistoricalData> getHourlyData(Context graphContext, String fsym, String tsym, int limit) {

        List<HourlyHistoricalData> hourlyHistoricalDataList = new ArrayList<>();
        final String HOURLY_DATA_URL = "https://min-api.cryptocompare.com/data/v2/histohour?fsym=";
        String url = HOURLY_DATA_URL + fsym + "&tsyms=" + tsym + "&limit" + limit;
        RequestQueue queue = Volley.newRequestQueue(graphContext);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url, null, response -> {
            try {
                JSONObject data = response.getJSONObject("Data");
                ...
                for (int iterator = 0; iterator <= dailyDataObjects.length(); iterator++) {
                    ...
                    double low = dailyData.getDouble("low");
                    double close = dailyData.getDouble("close");
                    HourlyHistoricalData hourlyHistoricalData = new HourlyHistoricalData(time, high, low, close);
                    hourlyHistoricalDataList.add(hourlyHistoricalData);
                }
        ...
        queue.add(jsonObjectRequest);
        return hourlyHistoricalDataList;
    }
    public List<MinuteHistoricalData> getMinuteData(Context graphContext, String fsym, String tsym, int limit){

        List<MinuteHistoricalData> minuteHistoricalDataList = new ArrayList<>();
        final String HOURLY_DATA_URL ="https://min-api.cryptocompare.com/data/v2/histominute?fsym=";
        String url =HOURLY_DATA_URL+fsym+"&tsyms="+tsym+"&limit"+limit;
        RequestQueue queue = Volley.newRequestQueue(graphContext);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url, null, response -> {
            try {
                ...
                for(int iterator =0;iterator<=dailyDataObjects.length();iterator++){
                    ...
                    double low = dailyData.getDouble("low");
                    double close = dailyData.getDouble("close");
                    MinuteHistoricalData minuteHistoricalData = new MinuteHistoricalData(time,high,low,close);
                    minuteHistoricalDataList.add(minuteHistoricalData);
                }
        ...
        queue.add(jsonObjectRequest);
        return minuteHistoricalDataList;
    }
    }
